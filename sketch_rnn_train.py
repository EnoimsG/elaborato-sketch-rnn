...
def evaluate_model(sess, model, data_set):
  ...
  for batch in range(data_set.num_batches):
    ...
    (cost, r_cost, penalty_cost, wae_penalty_cost) = sess.run([model.cost, model.r_cost, model.penalty_cost, model.wae_penalty_cost], feed)
    total_cost += cost
    total_r_cost += r_cost
    total_penalty_cost += penalty_cost
    total_wae_penalty_cost  += wae_penalty_cost
    
  total_cost /= (data_set.num_batches)
  total_r_cost /= (data_set.num_batches)
  total_penalty_cost /= (data_set.num_batches)
  total_wae_penalty_cost /= (data_set.num_batches)
  return (total_cost, total_r_cost, total_penalty_cost, total_wae_penalty_cost)

def get_z_batch_sample(sess, model, data_set):
  samples_to_encode = [data_set.random_sample() for _ in range(model.hps.batch_size)]
  return encodeBatch(model, sess, samples_to_encode)

def encodeBatch(model, sess, samples_to_encode):
  final = np.zeros((model.hps.batch_size, model.hps.max_seq_len+1, 5), dtype=float)
  seq_len = []
  for i, sample in enumerate(samples_to_encode):
    tmp = utils.to_big_strokes(sample, model.hps.max_seq_len).tolist()
    tmp.insert(0, [0, 0, 1, 0, 0])
    final[i, :, :] = tmp
    seq_len.append(len(sample))
  return sess.run(model.batch_z, feed_dict={model.input_data: final, model.sequence_lengths: seq_len})
...
def train(sess, model, eval_model, train_set, valid_set, test_set):
  ...
  for _ in range(hps.num_steps):
    ...
    if hps.penalty == 'KL':
      penalty_weight = hps.kl_weight 
      penalty_weight_start = hps.kl_weight_start
      penalty_decay_rate = hps.kl_decay_rate
    elif hps.penalty == 'WAE' or hps.penalty == 'WAE-K':
      penalty_weight = hps.wasserstein_weight
      penalty_weight_start = hps.wasserstein_weight_start
      penalty_decay_rate = hps.wasserstein_decay_rate
    else:
      assert False, "Please, select a valid penalty"

    curr_penalty_weight = (penalty_weight - (penalty_weight - penalty_weight_start) *
                      (penalty_decay_rate)**step)
    ...
    feed = {
        ...
        model.penalty_weight: curr_penalty_weight
    }

    (train_cost, r_cost, penalty_cost, _, train_step, _, wae_penalty_cost) = sess.run([
        model.cost, model.r_cost, model.penalty_cost, model.final_state,
        model.global_step, model.train_op, model.wae_penalty_cost
    ], feed)
    ...
    if step % 20 == 0 and step > 0:
      ...
      penalty_summ = tf.summary.Summary()
      penalty_summ.value.add(tag='Train_'+penalty_name+'_Cost (non pesata)', simple_value=float(penalty_cost))
      wae_penalty_summ = tf.summary.Summary()
      wae_penalty_summ.value.add(tag='Train_'+penalty_name+'_Cost (pesata)', simple_value=float(wae_penalty_cost))

      lr_summ = tf.summary.Summary()
      lr_summ.value.add(
          tag='Learning_Rate', simple_value=float(curr_learning_rate))
      penalty_weight_summ = tf.summary.Summary()
      penalty_weight_summ.value.add(
          tag=penalty_name+'_Weight', simple_value=float(curr_penalty_weight))
      time_summ = tf.summary.Summary()
      time_summ.value.add(
          tag='Time_Taken_Train', simple_value=float(time_taken))

      output_format = ('step: %d, lr: %.6f, penalty_w: %0.4f, cost: %.4f, '
                       'recon: %.4f, penalty: %.4f, train_time_taken: %.4f')
      output_values = (step, curr_learning_rate, curr_penalty_weight, train_cost,
                       r_cost, penalty_cost, time_taken)
      ...
      summary_writer.add_summary(penalty_summ, train_step)
      summary_writer.add_summary(wae_penalty_summ, train_step)
      summary_writer.add_summary(lr_summ, train_step)
      summary_writer.add_summary(penalty_weight_summ, train_step)
      summary_writer.add_summary(time_summ, train_step)
      summary_writer.flush()
      start = time.time()

    if step % hps.plot_every == 0 and step > 0:
      z = get_z_batch_sample(sess, eval_model, valid_set)
      np.save(FLAGS.log_root+'/z_batch_sample/'+str(step)+'.npy', z)

    if step % hps.save_every == 0 and step > 0:

      (valid_cost, valid_r_cost, valid_penalty_cost, valid_wae_penalty_cost) = evaluate_model(sess, eval_model, valid_set)
      ...
      valid_penalty_summ = tf.summary.Summary()
      valid_penalty_summ.value.add(
          tag='Valid_'+penalty_name+'_Cost (non pesata)', simple_value=float(valid_penalty_cost))
      valid_wae_penalty_summ = tf.summary.Summary()
      valid_wae_penalty_summ.value.add(
          tag='Valid_'+penalty_name+'_Cost (pesata)', simple_value=float(valid_wae_penalty_cost))
      ...
      output_format = ('best_valid_cost: %0.4f, valid_cost: %.4f, valid_recon: '
                       '%.4f, valid_penalty: %.4f, valid_time_taken: %.4f')
      output_values = (min(best_valid_cost, valid_cost), valid_cost,
                       valid_r_cost, valid_penalty_cost, time_taken_valid)
      output_log = output_format % output_values
      ...
      summary_writer.add_summary(valid_penalty_summ, train_step)
      summary_writer.add_summary(valid_time_summ, train_step)
      summary_writer.add_summary(valid_wae_penalty_summ, train_step)
      summary_writer.flush()
      ...
      if valid_cost < best_valid_cost:
        ...
        (eval_cost, eval_r_cost, eval_penalty_cost, eval_wae_penalty_cost) = evaluate_model(
        ...
        eval_penalty_summ = tf.summary.Summary()
        eval_penalty_summ.value.add(
            tag='Eval_'+penalty_name+'_Cost (non pesata)', simple_value=float(eval_penalty_cost))
        eval_wae_penalty_summ = tf.summary.Summary()
        eval_wae_penalty_summ.value.add(
            tag='Eval_'+penalty_name+'_Cost (pesata)', simple_value=float(eval_wae_penalty_cost))
        ...
        output_format = ('eval_cost: %.4f, eval_recon: %.4f, '
                         'eval_penalty: %.4f, eval_time_taken: %.4f')
        output_values = (eval_cost, eval_r_cost, eval_penalty_cost, time_taken_eval)
        output_log = output_format % output_values
        ...
        summary_writer.add_summary(eval_penalty_summ, train_step)
        summary_writer.add_summary(eval_time_summ, train_step)
        summary_writer.add_summary(eval_wae_penalty_summ, train_step)
        summary_writer.flush()
...
def trainer(model_params):
  ...
  #Set cuda device
  os.environ["CUDA_VISIBLE_DEVICES"]=model_params.gpu_id
  ...
  # create the folder for batch z
  tf.gfile.MakeDirs(FLAGS.log_root+'/z_batch_sample')
