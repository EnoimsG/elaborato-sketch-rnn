...
def get_default_hparams():
  ...
  hparams = contrib_training.HParams(
      ...
      z_size=128,  # Size of latent vector z.
      batch_size=100,  # batch size.
      grad_clip=1.0,  # Gradient clipping. 
      num_mixture=20,  # Number of mixtures in Gaussian mixture model.
      learning_rate=0.001,  # Learning rate.
      decay_rate=0.9999,  # Learning rate decay per batch.
      ...
      use_output_dropout=False,  # Output dropout.
      ...
      penalty="WAE-K", # current penalty, WAE or KL or WAE-K

      # KL
      kl_weight=1.0,  # KL weight of loss equation.
      kl_weight_start=0.1,  # KL start weight when annealing.
      kl_tolerance=0.2,  # Level of KL loss at which to stop optimizing for KL.
      kl_decay_rate=0.99995,  # KL annealing decay rate per minibatch.

      # Wasserstein
      wasserstein_weight=20.0, # Wasserstein weight of loss equation
      wasserstein_weight_start=10.0, # Wasserstein start weight when annealing
      wasserstein_tolerance=0., # Level of Wasserstein loss at which to stop optimizing.
      wasserstein_decay_rate=0.99995,  # decay rate per minibatch
      pz_scale=1., # Pz scale
      wae_kernel='RBF', # Kernel to use in MMD, can be RBF o IMQ

      #Utils
      gpu_id="0", #  GPU id of mulambda to use
      plot_every=5000
  )
  return hparams
...
class Model(object):
  ...
  def build_model(self, hps):
    ...
    if hps.conditional:  # conditional: encoder is present
    ...
    if hps.conditional: # conditional mode
      ...
      if hps.penalty == 'KL':
        # KL cost
        self.penalty_cost = -0.5 * tf.reduce_mean(
            (1 + self.presig - tf.square(self.mean) - tf.exp(self.presig)))
        self.penalty_cost = tf.maximum(self.penalty_cost, self.hps.kl_tolerance)
      elif hps.penalty == 'WAE':
        self.penalty_cost = tf.reduce_mean(
          tf.math.sqrt(tf.math.square(tf.norm(self.mean, ord=2, axis=1)) +
                            tf.math.reduce_sum(self.sigma_squared + 1 - 2 * tf.math.sqrt(self.sigma_squared), axis=1)))
        tf.debugging.assert_positive(self.sigma_squared, message='self.sigma_squared is negative')
        tf.debugging.check_numerics(self.sigma_squared, message='self.sigma_squared has some NaN')
        
        tf.debugging.check_numerics(self.penalty_cost, message='penalty_cost has some Nan')
        tf.debugging.assert_positive(self.penalty_cost, message='penalty_cost is Negative')

        #self.penalty_cost = tf.maximum(self.penalty_cost, self.hps.wasserstein_tolerance)
      elif self.hps.penalty == 'WAE-K':
        sample_qz = tfp.distributions.MultivariateNormalDiag(loc=self.mean, scale_diag=self.sigma_squared, allow_nan_stats=False).sample()
        sample_pz = tfp.distributions.MultivariateNormalDiag(loc=tf.zeros([self.hps.batch_size, self.hps.z_size]), scale_diag=tf.ones([self.hps.batch_size, self.hps.z_size]), allow_nan_stats=False).sample()
        #sample_pz = tf.random_normal((self.hps.batch_size, self.hps.z_size), 0.0, 1.0, dtype=tf.float32)
        self.penalty_cost = wasserstein_utils.mmd_penalty(sample_qz, sample_pz, self.hps)
      else:
        assert False, 'Please, select a valid penalty: WAE, WAE-K or KL'
      ...
      self.penalty_cost = tf.zeros([], dtype=tf.float32)
    ...
    if self.hps.is_training:
      ...
      if self.hps.penalty == 'KL':
        self.penalty_weight = tf.Variable(self.hps.kl_weight_start, trainable=False)
        self.cost = self.r_cost + self.penalty_cost * self.penalty_weight
        self.wae_penalty_cost = self.penalty_cost * self.penalty_weight  # WORKAROUND
      elif self.hps.penalty == 'WAE':
        self.penalty_weight = tf.Variable(self.hps.wasserstein_weight_start, trainable=False)
        self.wae_penalty_cost = self.penalty_weight * tf.maximum(self.penalty_cost, self.hps.wasserstein_tolerance)
        self.cost = self.r_cost + self.wae_penalty_cost
      elif self.hps.penalty == 'WAE-K':
        self.penalty_weight =  tf.Variable(self.hps.wasserstein_weight_start, trainable=False)
        self.wae_penalty_cost = self.penalty_weight * tf.maximum(self.penalty_cost, self.hps.wasserstein_tolerance)
        self.cost = self.r_cost + self.wae_penalty_cost
